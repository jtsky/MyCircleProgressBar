...
public class RiseNumberTextView extends TextView implements RiseNumberBase {
	...
	private void runDouble() {
		ValueAnimator valueAnimator = ValueAnimator.ofFloat((float)fromNumber, (float)number);
		...
		valueAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					...
					public void onAnimationUpdate(ValueAnimator valueAnimator) {
						...
						setText(fnum.format(Double.parseDouble(valueAnimator
	...
	public RiseNumberTextView withNumber(float number) {
		...
		numberType = 2;
		if (number > 1000) {
			fromNumber = number- (double) Math.pow(10, sizeOfInt((int) number) - 2);
		} else {
			fromNumber = number / 2;
		}
